...
- Repository Providerの業務失敗を成功扱いする問題と、CLIが業務失敗でも終了コード0を返す問題を修正しました。構造化・テキスト応答の検証と回帰テストを追加しました。稼働サービスには未適用です。

- Codex／Claude Codeのユーザー単位の接続登録・解除CLI、インストーラのクライアント・プロファイル選択、所有設定の保護と失敗時復元を追加しました。MSI実機操作と画面確認は公開前の検証項目です。
