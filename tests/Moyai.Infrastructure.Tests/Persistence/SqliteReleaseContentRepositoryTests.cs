...
public sealed class SqliteReleaseContentRepositoryTests
{
    ...
    public async Task AddListAndRemovePersistContentAndAuditEvents()
    {
        ...
        try
        {
            ...
            Guid buildArtifactId = Guid.NewGuid();
            ReleaseWorkItem relation = ReleaseWorkItem.Create(projectId, releaseId, Guid.NewGuid(), "fixes", TimeProvider.System);
            ReleaseArtifact artifact = ReleaseArtifact.Create(projectId, releaseId, buildArtifactId, "Windows MSI", "installer", "windows", "x64", "Moyai.msi", "dist/Moyai.msi", null, 2048, "abc", "dist/Moyai.msi.sig", null, TimeProvider.System);

            await SeedParentsAsync(connectionString, projectId, releaseId, relation.WorkItemId, buildArtifactId);
            ...
            ReleaseArtifact persistedArtifact = Assert.Single(await repository.ListArtifactsAsync(projectId, releaseId));
            Assert.Equal(artifact, persistedArtifact);
            Assert.Equal(buildArtifactId, persistedArtifact.BuildArtifactId);
    ...
    private static async Task SeedParentsAsync(string connectionString, Guid projectId, Guid releaseId, Guid workItemId, Guid buildArtifactId)
    {
        await using var connection = new SqliteConnection(connectionString);
        await connection.OpenAsync();
        await using SqliteCommand command = connection.CreateCommand();
        command.CommandText = "INSERT INTO projects(id,name,source_path,repository_url,repository_provider,build_provider,deploy_mode,git_remote_name,created_at,updated_at,revision) VALUES($project,'Test','src','https://example.test/repository','github','local','local','origin',$now,$now,1); INSERT INTO releases(id,project_id,version,channel,status,revision,created_at,updated_at) VALUES($release,$project,'1.0.0','stable','draft',1,$now,$now); INSERT INTO work_items(id,project_id,type,key,sequence_no,title,status,priority,created_by_type,created_by_name,revision,created_at,updated_at) VALUES($item,$project,'bug','BUG-1',1,'Bug','open','medium','agent','test',1,$now,$now); INSERT INTO builds(id,project_id,provider,status,source_commit,configuration,actor_type,actor_name,created_at,revision) VALUES($build,$project,'local','succeeded','abc123','Release','agent','test',$now,1); INSERT INTO build_artifacts(id,project_id,build_id,name,artifact_type,artifact_kind,file_path,file_size,sha256,created_at) VALUES($build_artifact,$project,$build,'Moyai MSI','installer','file','dist/Moyai.msi',2048,'abc',$now);";
        ...
        command.Parameters.AddWithValue("$build", Guid.NewGuid().ToString("D"));
        command.Parameters.AddWithValue("$build_artifact", buildArtifactId.ToString("D"));
